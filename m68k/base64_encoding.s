...
in_buffer:       .byte  '_________________________________________________________________' ; [0; 0x40]
...
_start:
    ...
    movea.l  buffer, A2
    movea.l  lookup, A3
    movea.l  in_buffer, A4

    ; This loop is ONLY needed because wrench gives 2000 ops limit.
    ; Because of that I cant process input online, as my program for that needed ~2300 ops in the worst case scenario.
in_to_buffer_loop:
    cmp.l    0x40, D7
    bge      overflow
    add.l    1, D7
    ...
    move.b   D0, (A4)+
    cmp.b    0x0A, D0
    beq      in_to_buffer_done

    jmp      in_to_buffer_loop
...
loop:
    ...
    ; In this inner loop we load bytes from input in threes.
